...
def crea_analyst(max_iter=12):
    return Agent(
        ...
        backstory=(
            ...
            "BONUS DI SCORE (applica SEMPRE questi aggiustamenti prima di assegnare il punteggio finale):\n"
            "+2 se il prospect opera in: valvole industriali, oil & gas, compressori, impianti energetici "
            "o rinnovabili — settori prioritari 2026 con massima allocazione budget commerciale.\n"
            "+1 se il prospect è localizzato in Middle East (Arabia Saudita, UAE, Kuwait, Qatar, Oman, GCC) "
            "— area di espansione geografica prioritaria 2026.\n"
            "+1 se il prospect opera in ambienti che richiedono materiali anticorrosione "
            "(offshore, idrogeno, CCUS, petrolchimico aggressivo) — massima valorizzazione "
            "superleghe Inconel/Hastelloy e inox duplex.\n"
            "Il punteggio finale non può superare 10."
        ),
...
def crea_task_analisi(agente, lista_aziende, settore, area):
    return Task(
        description=(
            ...
            f"   BONUS OBBLIGATORI (applica prima di finalizzare lo score):\n"
            f"   +2 se opera in valvole industriali, oil & gas, compressori o energia/rinnovabili\n"
            f"   +1 se è in Middle East (Arabia Saudita, UAE, Kuwait, Qatar, Oman)\n"
            f"   +1 se opera in ambienti corrosivi (offshore, idrogeno, CCUS, petrolchimico)\n"
            f"   Il punteggio finale non può superare 10.\n"
            f"4. Scrivi una motivazione concisa per lo score (2-3 frasi), citando se possibile "
            f"il range di diametri, i materiali probabilmente usati e gli eventuali bonus applicati.\n\n"
